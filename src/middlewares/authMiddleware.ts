...
export const authMiddleware = (
  ...
  try {
    ...
    const decoded: any = jwt.verify(token, JWT_SECRET);
    // :white_check_mark: Fix: support tokens that store 'userId', '_id', or 'id'
    const userId = decoded.userId || decoded._id || decoded.id;
    if (!userId) {
      return res
        .status(400)
        .json({ message: "Invalid token structure, user ID missing." });
    }
    // :white_check_mark: Attach user data to req.user (standard pattern)
    (req as any).user = {
      id: userId,
      email: decoded.email,
